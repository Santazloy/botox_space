...
BRAND_KEYWORDS = [
    "HydroPeptide", "Medik8", "Image", "Is Clinical",
    "Colorescience", "Revitalash", "Academie", "Histolab"
]
SUBGROUP_KEYWORDS = ["Сироватки", "Маски", "Крем", "SPF", "Очищення", "Eye"]
PATTERN = r"(?i)(" + "|".join(BRAND_KEYWORDS + SUBGROUP_KEYWORDS) + r")"


@router.message(F.text.regexp(r"(?i)косметика|бренд|засіб|сировотка|мазі|крем"))
...
async def on_brand(callback: types.CallbackQuery):
    ...
    if not subs:
        sec = next(
            s for s in SUBGROUP_DEFINITIONS
            if s["brand"] == brand_id and s["id"].endswith("_all")
        )
        lines = [f"<b>{sec['emoji']} {sec['title']}</b>", ""] + sec["raw_text"].splitlines()
...
async def on_sub(callback: types.CallbackQuery):
    ...
    # меняем caption на заголовок подгруппы
    await callback.message.edit_caption(
    ...
    for idx, name in enumerate(items):
        builder.button(
            text=name,
            callback_data=f"cosm_prod:{sub_id}:{idx}"
        )
...
async def on_product(callback: types.CallbackQuery):
    ...
    idx = int(idx)
    sec = next(s for s in SUBGROUP_DEFINITIONS if s["id"] == sub_id)
    products = [ln.strip() for ln in sec["raw_text"].splitlines() if ln.strip()]
    product_name = products[idx]

    user_id = callback.from_user.id
    await db.add_message_to_history(user_id, "user", f"Опис продукту: {product_name}")

    await callback.bot.send_chat_action(callback.message.chat.id, ChatAction.TYPING)
    reply = await generate_cosmetics_reply_ua(user_id, f"Опис продукту: {product_name}")
    await db.add_message_to_history(user_id, "assistant", reply)

    # готовый Markdown-текст с заголовком и описанием
    md = (
        f"**_{product_name}_**\n\n"
        f"{reply}\n\n"
    ...
    # кнопка «Назад» в ту же подгруппу
    builder = InlineKeyboardBuilder()
